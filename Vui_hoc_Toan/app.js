...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function solveMathOrExplainLocally({ prompt, mode, studentName, grade, context }) {
    ...
    if (mode === "evaluate") {
      ...
      const userAns = (context?.studentAnswer || prompt || "").trim();
...
- Em hãy lật thẻ **Flashcard 3D** hoặc làm bài **Thử thách Điền khuyết** để khắc sâu kiến thức nhé! Định lý này đã được tự động lưu vào **Hộp Đỏ (Hộp 1)** để nhắc em ôn lại.`;
      ...
      keywords.forEach(kw => {
        ...
        if (userAnsLower.includes(kwClean) || (kwClean.length > 3 && userAnsLower.includes(kwClean.slice(0, Math.floor(kwClean.length * 0.8))))) {
          matchedCount++;
          matchedList.push(kw);
        } else {
          missedList.push(kw);
      ...
      const ratio = matchedCount / totalKws;
      ...
      if (ratio >= 0.7 || userAnsLower.includes(std.toLowerCase().slice(0, 30))) {
        score = Math.min(10, (9.0 + Math.random() * 1.0)).toFixed(1);
        badgeClass = "score-badge-high";
        comment = `🌟 **Xuất sắc!** Câu trả lời của ${studentName} rất chuẩn xác, nắm vững bản chất toán học của định lý **${thmTitle}**!`;
      } else if (ratio >= 0.35) {
        score = (5.5 + ratio * 3.0).toFixed(1);
        badgeClass = "score-badge-med";
        comment = `👍 **Khá tốt!** ${studentName} đã nắm được ý chính của định lý, tuy nhiên còn thiếu một số ý/điều kiện quan trọng.`;
      } else {
        // Kiểm tra xem có từ toán học liên quan không
        ...
        if (hasSomeMath) {
          score = (3.0 + Math.random() * 1.5).toFixed(1);
          badgeClass = "score-badge-low";
          comment = `⚠️ ${studentName} có đề cập đến một số khái niệm liên quan nhưng cách phát biểu chưa chính xác hoặc còn nhầm lẫn.`;
        } else {
          score = (1.0 + Math.random() * 1.0).toFixed(1);
          badgeClass = "score-badge-low";

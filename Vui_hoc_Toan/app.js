...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const GRADE_STORAGE_KEY = "vui_hoc_toan_selected_grade_v2";

  function getStoredGrade(fallback = 7) {
    try {
      const g = localStorage.getItem(GRADE_STORAGE_KEY) || localStorage.getItem("vui_hoc_toan_selected_grade");
      if (g === "6" || g === 6 || g === 6) return 6;
      if (g === "7" || g === 7 || g === 7) return 7;
    } catch (e) {}
    return fallback;
  }

  function ensureLeitnerIntegrity(targetGrade) {
    if (!appState || !appState.leitner) {
      if (appState) appState.leitner = { red: [], yellow: [], green: [] };
      else return;
    }
    if (!Array.isArray(appState.leitner.red)) appState.leitner.red = [];
    if (!Array.isArray(appState.leitner.yellow)) appState.leitner.yellow = [];
    if (!Array.isArray(appState.leitner.green)) appState.leitner.green = [];

    if (typeof THEOREMS_DATA !== "undefined" && Array.isArray(THEOREMS_DATA)) {
      const gradeThms = THEOREMS_DATA.filter(thm => thm.grade === targetGrade);
      const existing = new Set([
        ...appState.leitner.red,
        ...appState.leitner.yellow,
        ...appState.leitner.green
      ]);
      gradeThms.forEach(thm => {
        if (!existing.has(thm.id)) {
          appState.leitner.red.push(thm.id);
        }
      });
    }
  }
  ...
  function loadUserAndState() {
    try {
      const activeGrade = getStoredGrade(7);
      if (currentUsername && appUsers[currentUsername]) {
        ...
        const userGrade = getStoredGrade(currentUser.grade || activeGrade || 7);
        const defaultState = createDefaultState(userGrade);
  ...
  function formatAIResponseHTML(text) {
    ...
    cleanText = cleanText
      .replace(/^#### (.*$)/gim, '<h5 class="ai-hd" style="margin: 8px 0 3px; color: var(--primary); font-weight:700; font-size:14px;">$1</h5>')
      .replace(/^### (.*$)/gim, '<h4 class="ai-hd" style="margin: 10px 0 4px; color: var(--primary); font-weight:750; font-size:15px;">$1</h4>')
      .replace(/^## (.*$)/gim, '<h3 class="ai-hd" style="margin: 12px 0 6px; color: var(--primary); font-weight:800; font-size:16px;">$1</h3>')
      .replace(/^# (.*$)/gim, '<h2 class="ai-hd" style="margin: 14px 0 8px; color: var(--primary); font-weight:800; font-size:18px;">$1</h2>');

    // 3. Blockquotes (> text)
    cleanText = cleanText.replace(/^\> (.*$)/gim, '<blockquote>$1</blockquote>');

    // 4. Định dạng Bold, Italic, Strikethrough, Inline Code
    // Bold + Italic: ***text*** hoặc ___text___
    cleanText = cleanText.replace(/(\*\*\*|___)([\s\S]*?)\1/g, '<strong><em>$2</em></strong>');
    // Bold: **text** hoặc __text__
    cleanText = cleanText.replace(/(\*\*|__)([\s\S]*?)\1/g, '<strong>$2</strong>');
    // Italic: *text* hoặc _text_
    cleanText = cleanText.replace(/(^|[^\*])\*([^\*\n]+)\*([^\*]|$)/g, '$1<em>$2</em>$3');
    cleanText = cleanText.replace(/(^|[^_])_([^_\n]+)_([^_]|$)/g, '$1<em>$2</em>$3');
    // Strikethrough: ~~text~~
    cleanText = cleanText.replace(/~~(.*?)~~/g, '<del>$1</del>');
    // Inline Code: `code`
    cleanText = cleanText.replace(/`([^`\n]+)`/g, '<code class="ai-inline-code">$1</code>');

    // Xóa triệt để các dấu ** hoặc * đơn lẻ không có cặp đóng
    cleanText = cleanText.replace(/\*\*/g, '');

    // 5. Danh sách đầu dòng (- item hoặc * item hoặc 1. item)
    cleanText = cleanText.replace(/^[•\-\*]\s+(.*$)/gim, '<li>$1</li>');
    cleanText = cleanText.replace(/^(\d+)\.\s+(.*$)/gim, '<li>$2</li>');

    // 6. Ngắt dòng
    cleanText = cleanText.replace(/\n/g, '<br>');

    // 7. Khôi phục lại công thức KaTeX nguyên vẹn (sử dụng split/join)
  ...
  // ---------------------------------------------------------------
  // 3.1.1. BỘ CHUẨN HÓA VĂN BẢN TOÁN HỌC & TỪ ĐỒNG NGHĨA LINH HOẠT
  // ---------------------------------------------------------------
  ...
  const SYNONYMS_MAP = [
    { keys: ["90 độ", "90", "vuông", "góc vuông"], canonical: "90 độ" },
    { keys: ["180 độ", "180", "hai góc vuông"], canonical: "180 độ" },
    { keys: ["60 độ", "60"], canonical: "60 độ" },
    { keys: ["chẵn", "0 2 4 6 8", "0, 2, 4, 6, 8", "chữ số chẵn", "số chẵn"], canonical: "chẵn" },
    { keys: ["0", "số 0", "chữ số 0", "không"], canonical: "0" },
  ...
  function checkBlankAnswer(userRaw, expectedRaw) {
    ...
    const expectedList = Array.isArray(expectedRaw) ? expectedRaw : [expectedRaw];
    const userNorm = normalizeMathText(userRaw);

    for (const exp of expectedList) {
      const expNorm = normalizeMathText(exp);

      // 1. Exact match sau khi chuẩn hóa
      if (userNorm === expNorm) return true;

      // 2. Tra cứu nhóm từ đồng nghĩa (Synonyms)
      for (const group of SYNONYMS_MAP) {
        const userInGroup = group.keys.some(k => normalizeMathText(k) === userNorm || userNorm.includes(normalizeMathText(k)));
        const expInGroup = group.keys.some(k => normalizeMathText(k) === expNorm || expNorm.includes(normalizeMathText(k)));
        if (userInGroup && expInGroup) return true;
      }

      // 3. Kiểm tra chứa nhau (Inclusion check)
      if (userNorm.length >= 3 && expNorm.includes(userNorm)) return true;
      if (expNorm.length >= 3 && userNorm.includes(expNorm)) return true;

      // 4. Bỏ qua dấu tiếng Việt / khoảng trắng nếu khớp các ký tự chính
      const uClean = userNorm.replace(/[^a-z0-9]/g, "");
      const eClean = expNorm.replace(/[^a-z0-9]/g, "");
      if (uClean.length >= 3 && (uClean === eClean || eClean.includes(uClean) || uClean.includes(eClean))) {
        return true;
      }
    }
  ...
  function solveMathOrExplainLocally({ prompt, mode, studentName, grade, context }) {
    ...
    // 0. CHẾ ĐỘ ĐÁNH GIÁ & CHẤM ĐIỂM VẤN ĐÁP ĐỊNH LÝ (ƯU TIÊN HÀNG ĐẦU KHI MODE === EVALUATE)
    if (mode === "evaluate") {
      const std = context?.standardAnswer || "";
      const thmTitle = context?.theoremTitle || "Định lý";
      const userAnsClean = (context?.studentAnswer || prompt || "").trim();
      const userAnsLower = userAnsClean.toLowerCase();
      const stdClean = (std || "").trim();
      const stdLower = stdClean.toLowerCase();

      // Danh sách các mẫu câu thể hiện không biết / chưa học / chịu / quên
      const unknowingPhrases = [
        "không biết", "ko biết", "k biết", "chưa biết", "khum biết",
        "không nhớ", "ko nhớ", "k nhớ", "chưa nhớ",
        "chưa học", "chịu", "chịu thôi", "em chịu", "chịu thua",
        "không rõ", "ko rõ", "k rõ", "chưa rõ",
        "tôi không biết", "em không biết", "em ko biết", "em k biết",
        "quên", "quên rồi", "quên mất", "em quên", "chưa học tới",
        "không hiểu", "ko hiểu", "k hiểu", "không làm được",
        "không có", "không nhớ gì", "chả biết", "chả nhớ",
        "nothing", "idk", "i don't know", "dont know"
      ];

      const isUnknowing = unknowingPhrases.some(phrase => userAnsLower.includes(phrase)) ||
        userAnsClean.length < 4 ||
        /^[^a-zA-Z0-9\u00C0-\u1EF9]+$/.test(userAnsClean);

      if (isUnknowing) {
        return `<div class="score-badge-low">⭐ Điểm số: 0 / 10 (Chưa nắm được)</div>

💡 **Nhận xét của Thầy**:
Không sao cả ${studentName} ơi! Việc chưa nhớ hay chưa thuộc định lý là hoàn toàn bình thường khi mới học. Thầy trò mình cùng nhau ôn lại ngay bây giờ nhé!

📖 **Phát biểu chuẩn SGK Kết nối tri thức**:
*"${std || "Xem lại nội dung định lý trong sách giáo khoa."}"*

${context?.formula ? `📐 **Công thức toán học cần nhớ**:\n$$${context.formula}$$` : ""}

🎯 **Lời khuyên cho ${studentName}**:
- Hãy đọc to định lý trên 2-3 lần để ghi nhớ vào trí nhớ dài hạn.
- Em hãy lật thẻ **Flashcard 3D** hoặc làm bài **Thử thách Điền khuyết** để khắc sâu kiến thức nhé! Định lý này đã được tự động lưu vào **Hộp Đỏ (Hộp 1)** để nhắc em ôn lại.`;
      }

      // Kiểm tra trùng khớp hoàn toàn / y hệt với đáp án chuẩn
      const userNorm = normalizeMathText(userAnsClean);
      const stdNorm = normalizeMathText(stdClean);

      if (userNorm === stdNorm || userAnsLower === stdLower || (userNorm.length >= 20 && stdNorm.includes(userNorm))) {
        return `<div class="score-badge-high">⭐ Điểm số: 10 / 10</div>

💡 **Nhận xét của Thầy**:
🌟 **Hoàn toàn chính xác tuyệt đối (10/10)!** ${studentName} đã phát biểu định lý **${thmTitle}** chuẩn từng câu chữ theo đúng sách giáo khoa Kết nối tri thức!

📖 **Phát biểu chuẩn SGK Kết nối tri thức**:
*"${std}"*

${context?.formula ? `📐 **Công thức toán học**:\n$$${context.formula}$$` : ""}`;
      }

      // Đánh giá dựa trên độ khớp từ khóa và ngữ nghĩa
      ...
      keywords.forEach(kw => {
        const kwNorm = normalizeMathText(kw);
        if (userNorm.includes(kwNorm) || (kwNorm.length > 3 && userNorm.includes(kwNorm.slice(0, Math.floor(kwNorm.length * 0.8))))) {
          matchedCount++;
          matchedList.push(kw);
        } else {
          missedList.push(kw);
      ...
      const totalKws = Math.max(1, keywords.length);
      const keywordRatio = matchedCount / totalKws;

      // Độ tương đồng văn bản (Token overlap)
      const userWords = userNorm.split(/\s+/).filter(w => w.length > 1);
      const stdWords = stdNorm.split(/\s+/).filter(w => w.length > 1);
      let overlapWords = 0;
      userWords.forEach(w => {
        if (stdWords.includes(w)) overlapWords++;
      });
      const textOverlapRatio = stdWords.length > 0 ? (overlapWords / stdWords.length) : 0;

      const combinedScore = keywordRatio * 0.6 + textOverlapRatio * 0.4;
      ...
      if (combinedScore >= 0.85 || keywordRatio === 1.0) {
        score = 10;
        badgeClass = "score-badge-high";
        comment = `🌟 **Xuất sắc 10/10!** ${studentName} đã phát biểu định lý **${thmTitle}** rất chính xác, đầy đủ toàn bộ ý và từ khóa trọng tâm!`;
      } else if (combinedScore >= 0.65 || keywordRatio >= 0.75) {
        score = (9.0 + (combinedScore - 0.65) * 2.5).toFixed(1);
        if (parseFloat(score) > 9.8) score = "9.8";
        badgeClass = "score-badge-high";
        comment = `🌟 **Rất xuất sắc!** Câu trả lời của ${studentName} rất chuẩn xác, nắm vững bản chất toán học của định lý **${thmTitle}**!`;
      } else if (combinedScore >= 0.35 || keywordRatio >= 0.4) {
        score = (6.0 + (combinedScore - 0.35) * 5.0).toFixed(1);
        badgeClass = "score-badge-med";
        comment = `👍 **Khá tốt!** ${studentName} đã nắm được các ý chính của định lý, tuy nhiên cần bổ sung thêm một số ý/điều kiện quan trọng.`;
      } else {
        const mathWords = ["tam giác", "góc", "bằng nhau", "song song", "cạnh", "chia hết", "tổng", "đối đỉnh", "lũy thừa", "phân số", "tỉ số", "bằng", "vuông", "đường thẳng"];
        const hasSomeMath = mathWords.some(w => userAnsLower.includes(w));
        if (hasSomeMath) {
          score = "3.5";
          badgeClass = "score-badge-low";
          comment = `⚠️ ${studentName} có đề cập đến một số khái niệm toán học nhưng cách phát biểu chưa chính xác hoặc còn nhầm lẫn.`;
        } else {
          score = "1.0";
          badgeClass = "score-badge-low";
          comment = `❌ Câu trả lời của ${studentName} chưa đúng trọng tâm của định lý **${thmTitle}**.`;
        }
      }

      let detailBreakdown = "";
      if (matchedList.length > 0) {
        detailBreakdown += `\n- ✅ **Ý đã nêu đúng**: ${matchedList.join(", ")}`;
      }
      if (missedList.length > 0) {
        detailBreakdown += `\n- 🔍 **Ý/Điều kiện còn thiếu**: ${missedList.join(", ")}`;
      }

      return `<div class="${badgeClass}">⭐ Điểm số: ${score} / 10</div>

💡 **Nhận xét của Thầy**:
${comment}${detailBreakdown}
...
3. ❓ **Bước 3**: Hãy gửi câu hỏi hoặc đề bài cụ thể hơn để Thầy gợi ý từng bước nhé!`;
  ...
  function syncGradeHeaderUI() {
    const curGrade = appState ? appState.selectedGrade : getStoredGrade(7);
    gradeBtns.forEach(b => {
      if (parseInt(b.dataset.grade) === curGrade) {
    ...
    if (brandBadge) {
      brandBadge.textContent = `SÁCH KẾT NỐI TRI THỨC VỚI CUỘC SỐNG • TOÁN LỚP ${curGrade}`;
    }
  }
  ...
  gradeBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      try {
        ...
        localStorage.setItem("vui_hoc_toan_selected_grade", grade);
        if (currentUser && !currentUser.isGuest && appUsers[currentUser.username]) {
          appUsers[currentUser.username].grade = grade;
          if (appUsers[currentUser.username].state) {
            appUsers[currentUser.username].state.selectedGrade = grade;
          }
          saveUsers();
        }
      } catch (e) {}
      // Tự động tải lại trang theo yêu cầu của người dùng để nạp lại toàn bộ nội dung sách KNTT lớp 6 hoặc 7
  ...
  function updateLeitnerCounts() {
    if (!appState || !appState.leitner) return;
    const currentGradeThmIds = new Set(THEOREMS_DATA.filter(t => t.grade === appState.selectedGrade).map(t => t.id));
    const redCount = (appState.leitner.red || []).filter(id => currentGradeThmIds.has(id)).length;
    const yellowCount = (appState.leitner.yellow || []).filter(id => currentGradeThmIds.has(id)).length;
    const greenCount = (appState.leitner.green || []).filter(id => currentGradeThmIds.has(id)).length;

    const elRed = document.getElementById("count-box-red");
    const elYellow = document.getElementById("count-box-yellow");
    const elGreen = document.getElementById("count-box-green");
  ...
  document.getElementById("btn-fillblank-check").addEventListener("click", () => {
    ...
    inputs.forEach(input => {
      ...
      const expected = thm.fillBlank.answers[idx];

      if (checkBlankAnswer(userVal, expected)) {
  ...
  document.getElementById("btn-submit-oral").addEventListener("click", async () => {
    ...
    const promptText = `Học sinh trả lời câu hỏi: "${thm.aiQuestion || thm.title}"\nCâu trả lời của học sinh: "${answer}"\nĐịnh lý chuẩn: "${thm.standardAnswer}"`;

    const studentName = (currentUser && !currentUser.isGuest) ? currentUser.fullname : "em";
    ...
    const reply = await callAIBackend({
      ...
      context: {
        ...
        formula: thm.formula || "",
        hypothesis: thm.hypothesis || "",
        conclusion: thm.conclusion || ""
      }
    });

    feedbackContent.innerHTML = formatAIResponseHTML(reply);
    renderAllMath(feedbackContent);

    // Cập nhật Leitner Box và âm thanh theo kết quả
    const isHighScore = reply.includes("score-badge-high") || reply.includes("Điểm số: 9") || reply.includes("Điểm số: 10") || reply.includes("Điểm số: 8.");
    const isMedScore = reply.includes("score-badge-med") || reply.includes("Điểm số: 6") || reply.includes("Điểm số: 7");

    if (isHighScore) {
      playCelebration();
      setLeitnerBox(thm.id, "green");
      addExpAndCoins(20, 10);
    } else if (isMedScore) {
      playSuccessSound();
      setLeitnerBox(thm.id, "yellow");
      addExpAndCoins(10, 5);
    } else {
  ...
  function applyGradeExamFilter() {
    const targetGradeStr = String(appState.selectedGrade || 7);
    if (examFilterBtns && examFilterBtns.length > 0) {
      examFilterBtns.forEach(b => {
        if (b.dataset.examFilter === `grade-${targetGradeStr}`) {
          b.classList.add("active");
        } else {
          b.classList.remove("active");
        }
    ...
    document.querySelectorAll(".exam-card").forEach(card => {
      ...
      card.style.display = (grade === targetGradeStr) ? "flex" : "none";
    });
  }
  ...
  function updateDashboardStats() {
    ...
    const currentGradeThmIds = new Set(THEOREMS_DATA.filter(t => t.grade === appState.selectedGrade).map(t => t.id));
    const greenCount = (appState.leitner.green || []).filter(id => currentGradeThmIds.has(id)).length;
    const learningCount = (appState.leitner.yellow || []).filter(id => currentGradeThmIds.has(id)).length + (appState.leitner.red || []).filter(id => currentGradeThmIds.has(id)).length;

    const elMastered = document.getElementById("dash-mastered-count");
    const elLearning = document.getElementById("dash-learning-count");
    const elExams = document.getElementById("dash-exams-count");
    const elStreak = document.getElementById("dash-streak-count");
    const elHeaderStreak = document.getElementById("header-streak-count");

    if (elMastered) elMastered.textContent = greenCount;
    if (elLearning) elLearning.textContent = learningCount;
    if (elExams) elExams.textContent = appState.examHistory ? appState.examHistory.length : 0;
    if (elStreak) elStreak.textContent = appState.streak;
    if (elHeaderStreak) elHeaderStreak.textContent = `${appState.streak} ngày`;
  ...
  function renderKnowledgeMap() {
    ...
    const currentGradeTheorems = THEOREMS_DATA.filter(thm => thm.grade === appState.selectedGrade);
    currentGradeTheorems.forEach(thm => {
      const box = getLeitnerBox(thm.id);
  ...
  // ---------------------------------------------------------------
    const STAGES_DATA_GRADE_6 = [
    {
      id: 1,
      realm: "ẢI 1 • SỐ HỌC (LỚP 6)",
      name: "Thung Lũng Lũy Thừa & Tập Hợp",
      summary: "Tập hợp số tự nhiên & Quy tắc nhân chia hai lũy thừa cùng cơ số",
      thmTarget: "toan6-hk1-tap-hop-luy-thua",
      challenge: {
        question: "Giá trị của biểu thức $2^3 \\cdot 2^2$ bằng:",
        options: [
          "A. $2^5 = 32$",
          "B. $2^6 = 64$",
          "C. $4^5 = 1024$",
        ...
        correctIndex: 0,
        explanation: "Theo quy tắc nhân hai lũy thừa cùng cơ số: $a^m \\cdot a^n = a^{m+n}$, ta có $2^3 \\cdot 2^2 = 2^{3+2} = 2^5 = 32$."
      }
    },
    {
      id: 2,
      realm: "ẢI 2 • TÍNH CHIA HẾT (LỚP 6)",
      name: "Vùng Đất Chia Hết & Ước Bội",
      summary: "Dấu hiệu chia hết cho 2, 5, 3, 9 & Ước chung lớn nhất (ƯCLN)",
    ...
    {
      id: 3,
      realm: "ẢI 3 • SỐ NGUYÊN (LỚP 6)",
      name: "Mê Cung Số Nguyên & Quy Tắc Dấu Ngoặc",
      summary: "Các phép tính với số nguyên âm và quy tắc đổi dấu khi bỏ ngoặc",
      thmTarget: "toan6-hk1-quy-tac-dau-ngoac",
      challenge: {
        question: "Khi bỏ dấu ngoặc có dấu trừ đằng trước: $-(a - b + c)$, ta được kết quả là:",
        options: [
          "A. $-a - b + c$",
          "B. $-a + b - c$",
          "C. $-a - b - c$",
        ...
        correctIndex: 1,
        explanation: "Quy tắc dấu ngoặc: Khi bỏ dấu ngoặc có dấu '-' đằng trước, ta phải đổi dấu tất cả các số hạng trong ngoặc: $-(a - b + c) = -a + b - c$."
      }
    },
    {
      id: 4,
      realm: "ẢI 4 • HÌNH HỌC TRỰC QUAN (LỚP 6)",
      ...
      challenge: {
        ...
        correctIndex: 2,
        explanation: "Tam giác đều có 3 cạnh bằng nhau nên chu vi = $3 \\times 6 = 18\\text{ cm}$."
      }
    },
    {
      id: 5,
      realm: "ẢI 5 • PHÂN SỐ TOÁN 6 (LỚP 6)",
      name: "Thác Nước Phân Số & Quy Đồng",
      summary: "Tính chất cơ bản của phân số, so sánh và các phép tính cộng trừ nhân chia",
      thmTarget: "toan6-hk2-tinh-chat-phan-so",
      challenge: {
        ...
        correctIndex: 1,
        explanation: "Nhân cả tử và mẫu của $\\frac{3}{5}$ với 3 ta được $\\frac{3 \\cdot 3}{5 \\cdot 3} = \\frac{9}{15}$."
      }
    },
    {
      id: 6,
      realm: "ẢI 6 • SỐ THẬP PHÂN & TỈ SỐ (LỚP 6)",
      name: "Đồng Cỏ Số Thập Phân & Tỉ Số %",
      summary: "Làm tròn số thập phân, tính tỉ số phần trăm và hai bài toán về phân số",
      thmTarget: "toan6-hk2-so-thap-phan-ti-so",
      challenge: {
        question: "Tìm $25\\%$ của số $120$:",
        options: [
          "A. 20",
          "B. 25",
          "C. 30",
        ...
        correctIndex: 2,
        explanation: "Giá trị $25\\%$ của 120 là: $120 \\cdot \\frac{25}{100} = 120 \\cdot 0{,}25 = 30$."
      }
    },
    {
      id: 7,
      realm: "ẢI 7 • HÌNH HỌC PHẲNG (LỚP 6)",
      name: "Đỉnh Núi Đoạn Thẳng & Trung Điểm",
      summary: "Điểm, đường thẳng, tia, trung điểm đoạn thẳng và số đo góc",
      thmTarget: "toan6-hk2-trung-diem-doan-thang",
      challenge: {
        question: "Cho đoạn thẳng $AB = 8\\text{ cm}$, điểm $M$ là trung điểm của $AB$. Độ dài đoạn thẳng $AM$ là:",
        options: [
          "A. 2 cm",
          "B. 4 cm",
          "C. 6 cm",
        ...
        correctIndex: 1,
        explanation: "Vì $M$ là trung điểm của $AB$ nên $AM = MB = \\frac{AB}{2} = \\frac{8}{2} = 4\\text{ cm}$."
      }
    }
  ];

  const STAGES_DATA_GRADE_7 = [
    {
      id: 1,
      realm: "ẢI 1 • SỐ HỮU TỈ (LỚP 7)",
      name: "Thung Lũng Số Hữu Tỉ & Số Thực",
      summary: "Các phép tính số hữu tỉ, quy tắc chuyển vế và căn bậc hai số học",
      thmTarget: "toan7-hk1-so-huu-ti-so-thuc",
      challenge: {
        question: "Kết quả của phép tính $\\left(-\\frac{2}{3}\\right)^2$ bằng:",
        options: [
          "A. $-\\frac{4}{9}$",
          "B. $\\frac{4}{9}$",
          "C. $-\\frac{4}{6}$",
        ...
        correctIndex: 1,
        explanation: "Lũy thừa bậc chẵn của một số âm luôn là số dương: $\\left(-\\frac{2}{3}\\right)^2 = \\frac{(-2)^2}{3^2} = \\frac{4}{9}$."
      }
    },
    {
      id: 2,
      realm: "ẢI 2 • GÓC & ĐƯỜNG THẲNG (LỚP 7)",
      name: "Vực Sâu Góc Đối Đỉnh & Tiên Đề Euclid",
      summary: "Định lý hai góc đối đỉnh, kề bù và Tiên đề Euclid về đường thẳng song song",
      ...
      challenge: {
        ...
        correctIndex: 1,
        explanation: "Hai góc đối đỉnh thì bằng nhau, do đó góc đối đỉnh với $\\widehat{O}_1$ cũng có số đo bằng 60°."
      }
    },
    {
      id: 3,
      realm: "ẢI 3 • ĐƯỜNG THẲNG SONG SONG (LỚP 7)",
      name: "Đại Lộ Song Song & Từ Vuông Góc Đến Song Song",
      summary: "Dấu hiệu nhận biết hai đường thẳng song song qua góc so le trong, đồng vị",
      thmTarget: "toan7-hk1-song-song-dau-hieu",
      challenge: {
        question: "Nếu đường thẳng $c$ cắt hai đường thẳng $a, b$ và trong các góc tạo thành có một cặp góc so le trong bằng nhau thì:",
        options: [
          "A. $a \\perp b$",
          "B. $a$ trùng $b$",
          "C. $a \\parallel b$ (song song)",
          "D. $a$ cắt $b$"
        ],
        correctIndex: 2,
        explanation: "Dấu hiệu nhận biết: Một đường thẳng cắt hai đường thẳng tạo thành một cặp góc so le trong bằng nhau thì hai đường thẳng đó song song ($a \\parallel b$)."
      }
    },
    {
      id: 4,
      realm: "ẢI 4 • TAM GIÁC BẰNG NHAU (LỚP 7)",
      name: "Đỉnh Núi Tam Giác Bằng Nhau & Tam Giác Cân",
      summary: "Tổng 3 góc tam giác (180°) & Các trường hợp bằng nhau (c-c-c, c-g-c, g-c-g)",
      thmTarget: "toan7-hk1-tam-giac-bang-nhau-3-truong-hop",
      challenge: {
        ...
        correctIndex: 0,
        explanation: "Tổng ba góc của tam giác luôn bằng 180°: $\\widehat{C} = 180^\\circ - (70^\\circ + 60^\\circ) = 50^\\circ$."
      }
    },
    {
      id: 5,
      realm: "ẢI 5 • TỈ LỆ THỨC & DÃY TỈ SỐ (LỚP 7)",
      name: "Hẻm Núi Tỉ Lệ Thức & Dãy Tỉ Số Bằng Nhau",
      summary: "Tích ngoại tỉ bằng tích trung tỉ, tính chất dãy tỉ số bằng nhau & Đại lượng tỉ lệ",
      thmTarget: "toan7-hk2-ti-le-thuc-day-ti-so",
      challenge: {
        question: "Từ tỉ lệ thức $\\frac{x}{3} = \\frac{y}{5}$ và $x + y = 16$, giá trị của $x$ và $y$ là:",
        options: [
          "A. $x = 6, y = 10$",
          "B. $x = 5, y = 11$",
          "C. $x = 8, y = 8$",
        ...
        correctIndex: 0,
        explanation: "Theo dãy tỉ số bằng nhau: $\\frac{x}{3} = \\frac{y}{5} = \\frac{x+y}{3+5} = \\frac{16}{8} = 2 \\Rightarrow x = 6, y = 10$."
      }
    },
    {
      id: 6,
      realm: "ẢI 6 • BIỂU THỨC ĐẠI SỐ (LỚP 7)",
      name: "Vương Quốc Đa Thức Một Biến & Nghiệm",
      summary: "Cộng trừ nhân chia đa thức một biến, xác định bậc và tìm nghiệm của đa thức",
      thmTarget: "toan7-hk2-bieu-thuc-da-thuc-mot-bien",
      challenge: {
        question: "Bậc của đa thức $P(x) = 4x^3 - 2x^2 + 5x - 7$ là:",
        options: [
          "A. 1",
          "B. 2",
          "C. 3",
        ...
        correctIndex: 2,
        explanation: "Bậc của đa thức một biến (thu gọn, khác đa thức 0) là số mũ lớn nhất của biến trong đa thức. Ở đây số mũ cao nhất là 3."
      }
    },
    {
      id: 7,
      realm: "ẢI 7 • QUAN HỆ HÌNH HỌC (LỚP 7)",
      name: "Thánh Địa Bất Đẳng Thức & Các Đường Đồng Quy",
      summary: "Bất đẳng thức tam giác, quan hệ góc - cạnh đối diện và 4 điểm đồng quy",
      thmTarget: "toan7-hk2-bat-dang-thuc-tam-giac",
      challenge: {
        ...
        correctIndex: 1,
        explanation: "Bất đẳng thức tam giác: Tổng độ dài hai cạnh bất kì luôn lớn hơn độ dài cạnh còn lại ($3 + 4 = 7 > 5$)."
      }
    }
  ];

  function getStagesData() {
    return (appState && appState.selectedGrade === 6) ? STAGES_DATA_GRADE_6 : STAGES_DATA_GRADE_7;
  }
    ...
    river.querySelectorAll(".btn-stage-action").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        if (action === "challenge") {
          ...
          const st = getStagesData().find(s => s.id === idNum);
          if (st) {
  ...
  function openStageChallenge(stageId) {
    const stage = getStagesData().find(s => s.id === stageId);
    if (!stage) return;
